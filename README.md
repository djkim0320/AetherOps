...
반복 횟수는 기본적으로 사용자가 지정하지 않습니다. 에이전트가 11단계 판단에서 근거 충분성, 검증 상태, evidence gap, 예산/시간 제약, 내부 안전 상한을 보고 자율적으로 결정합니다. 과거 API/테스트 호환을 위해 `autonomyPolicy.maxLoopIterations`는 optional safety cap으로만 허용하며, UI 설정 항목으로 노출하지 않습니다.

## Main Research Memory DB + Project Workspace DB

AetherOps는 장기 기억과 프로젝트 작업공간을 분리합니다. 현재 MVP 런타임은 별도 concrete store 두 개를 직접 노출하지 않고, `SqliteResearchStore`가 Main Research Memory와 Project Workspace를 함께 제공하는 composite facade 역할을 합니다. 물리 파일은 아래처럼 분리되어 생성되며, 향후 `SqliteMainResearchMemoryStore`와 `SqliteProjectWorkspaceStore`를 독립 구현으로 빼낼 수 있게 인터페이스를 유지합니다.

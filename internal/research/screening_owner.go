...
const (
	...
	engineeringScreeningOwnerPolicy  = "You are the single deterministic owner of every bundled engineering solver call required anywhere in the complete research plan, even when another workstream describes part of the numerical comparison. Inspect the complete plan before the first solver call, enumerate the whole requested case set, and execute every case in this one collector attempt. This includes every exact su2_cases member through su2_cfd and every XFOIL execution_purpose=screening job. Every SU2 input and physics choice comes from the exact plan contract, and no other normal collector may launch an engineering solver. Include every successful solver receipt artifact id returned in this attempt in engineering_receipt_artifact_ids and cite it from the corresponding claims; AetherOps rejects an XFOIL owner bundle that omits an executed screening receipt. Public-source work remains independent and may proceed normally."
	engineeringScreeningReadPolicy   = "You are not the engineering solver owner. Do not call su2_cfd, xfoil_polar, gmsh_wing_mesh, openvsp_wing_aero, or openvsp_modify_wing, and do not cite or copy a solver receipt produced by another collector attempt. Continue the assigned public-source research in parallel and use evidence_capture normally. The owner collector receives the complete plan and produces the single attempt-scoped solver receipt set."
	engineeringReceiptReadbackPolicy = "This is the sole core-authorized receipt-readback workstream for a REVIEW remediation. Do not execute or request approval for su2_cfd or any other solver. Call engineering_get exactly once for each job in reusable_engineering_results, using the current run_id and stage_attempt_id. Require reused_result=true, cite only the returned top-level receipt_artifact_id values, and report any identity, CAS, or metric mismatch instead of substituting a new calculation."
)

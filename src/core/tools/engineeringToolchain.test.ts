...
function settings(root: string, command = "xflr5"): EngineeringProgramSettings {
  return {
    ...
    openVsp: { enabled: false, command: "vspscript", scriptPath: "", workingDirectory: "", probeArgs: ["-help"], runArgsTemplate: ["-script", "{script}", "-spec", "{spec}", "-output", "{output}"], timeoutMs: 30 * 60_000 },
    xflr5: { enabled: true, command, scriptPath: "", workingDirectory: "", probeArgs: ["--help"], runArgsTemplate: ["--script", "{script}", "--spec", "{spec}", "--output", "{output}"], timeoutMs: 30 * 60_000 }
  };
}

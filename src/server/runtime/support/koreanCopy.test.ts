...
import { buildResearchReport } from "../../../core/output/report.js";
import { createDefaultSessions, seedResearchPlan } from "../../../core/input/researchSeed.js";
import { ResearchLoopStep, type ResearchProject, type ResearchSnapshot } from "../../../core/shared/types.js";

const forbiddenMojibake = new RegExp(["\\uFFFD", "[?]{3,}", "[\\uF900-\\uFAFF]", "[\\u0080-\\u009F]", "\\?[\\uAC00-\\uD7AF]"].join("|"), "u");

const forbiddenUtf8DecodeArtifacts = ["\uACF8\uBEC4", "\uC6D0\u0080", "\uF9DD", "\uAFBE\uC758", "\uC3F5\uC801", "\uC785\uC815"];
...
describe("Korean user-facing copy", () => {
  ...
  it("sends browser RPC payloads as explicit UTF-8 JSON", () => {
    const clientSource = readFileSync(join("src", "renderer", "platform", "rpcTransport.ts"), "utf8");
    ...
    expect(clientSource).toContain('new URL("/api/v2/rpc"');
    expect(clientSource).not.toContain("/api/rpc");
  ...
  it("keeps self-test Korean sentinel payloads readable", () => {
    const selftestSource = [
      join("scripts", "selftest", "rpc.mjs"),
      join("scripts", "selftest", "utf8-constants.mjs"),
      join("scripts", "selftest", "report.mjs")
    ]

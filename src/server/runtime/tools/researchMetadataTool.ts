...
export class ResearchMetadataTool implements ResearchTool {
  ...
  async run(input: ResearchToolInput, settings: AppSettings, context?: ResearchToolExecutionContext): Promise<ResearchToolResult> {
    ...
    return {
      toolRun: completedToolRun(
        input,
        this.name,
        startedAt,
        completedAt,
        { query, queries, provider: "openalex" },
        { sourceCount: sources.length, evidenceCount: evidence.length }
      ),
      evidence,
      artifacts: [],
      sources
...
async function fetchOpenAlexWorks(
  ...
  try {
    const parsed = JSON.parse(responseText) as OpenAlexResponse;
    return Array.isArray(parsed.results) ? parsed.results : [];
  } catch (error) {
    throw new Error(`OpenAlex metadata response was not valid JSON: ${error instanceof Error ? error.message : String(error)}`, {
      cause: error
    });
  }
}
